...
let mlist_of_list (lst : 'a list) : 'a mlist =
  List.fold_right (fun x y -> Cons (x,ref y)) lst (Nil);;
...
let rec loop (Cons (a,b) as m1) m2 =  
  match b with 
  | Nil -> Cons ()

let mappend m1 m2 = 
  match m1 with 
  | Nil -> raise (Invalid_argument "bad")
  | Cons (a,b) -> (match !b with 
                   | Nil -> Cons(a, ref m2)
                   | _ -> mappend (!b) m2);; 
